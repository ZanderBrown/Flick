...
from ctypes import POINTER, Array, Structure, c_char, c_int, c_short, c_uint16, c_ushort
...
import posix
from ctypes import c_int, create_string_buffer, sizeof, string_at
...
class I2CMaster(object):
    ...
    def __init__(self, n: int = default_bus, extra_open_flags: int = 0) -> None:
    ...
    def transaction(self, *msgs: i2c_msg) -> list[bytes]:
        """
...
def writing(addr: int, byte_seq: Sequence[int]) -> i2c_msg:
    """An I2C I/O message that writes one or more bytes of data.
...
def _new_i2c_msg(addr: int, flags: int, buf: Array[c_char]) -> i2c_msg:
    return i2c_msg(addr=addr, flags=flags, len=sizeof(buf), buf=buf)


def i2c_msg_to_bytes(m: i2c_msg) -> bytes:
